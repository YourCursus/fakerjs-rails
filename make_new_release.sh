...
DOCKER_IMAGE_NAME="$USER/fakerjs-rails"
LIBRARY_NEW_VERSION=`cat lib/**/*.rb | grep VERSION | awk '{ print $3 }' | tr -d "'"`

LIBRARY_UPDATED=`git status --porcelain | grep -v "lib/fakerjs/rails/version.rb"`
...
if [[ -z "$LIBRARY_UPDATED" ]]; then
  ...
  exit 1
elif [[ "$LIBRARY_UPDATED" == " M lib/fakerjs/rails/version.rb" ]]; then
